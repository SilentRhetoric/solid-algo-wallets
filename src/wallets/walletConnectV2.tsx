...
function useWalletConnect(): WalletInterface {
  ...
  const metadata = {
    name: 'Solid Algo Wallets Example App',
    description: 'Solid Algo Wallets Example App',
    url: 'https://github.com/SilentRhetoric/solid-algo-wallets#readme',
    icons: ['https://www.solidjs.com/img/logo/without-wordmark/logo.png'],
  }
  ...
  const image = (
    <svg width="100%" height="32" viewBox="0 0 203 32" xmlns="http://www.w3.org/2000/svg">
      <g clip-path="url(#a)">
  ...
  async function reconnect(onDisconnect: () => void) {
    console.debug('WalletConnect: reconnect')
    const client = await getClient(onDisconnect)
  ...
  async function disconnect(onDisconnect: () => void) {
    console.debug('Disconnecting WalletConnect')
    const client = walletClient()
...
export default createRoot(useWalletConnect)
